...
/// </summary>
public class QeBookmarksAndValueCompare
{
    private readonly ILogger<QeBookmarksCompare> _logger;

    public QeBookmarksAndValueCompare(ILogger<QeBookmarksCompare> logger)
    {
        _logger = logger;
    }

    [Function("QeBookmarksAndValueCompare")]
    public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequest req, ClaimsPrincipal principal)
    {
        ...
        try
        {
            ...
            response = BookmarkCompareHandler.CompareBookmarksAndValues(compareRequest);
        }
        catch (Exception ex)
        {
