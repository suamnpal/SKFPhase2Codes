...
{
    public class HYLSOR_Avdragshylsor_LW_7439149_V21 : ITemplateCalculations
    {
        private static readonly string[] Machines = new[] { "VTR-160", "MacTurn 550" };

        private const double TmpL = 540.0;
        private const double Tmpd = 548.0;
        private const double Tmpd1 = 525.0;
        private const double Amatt = 19.0;
        private const double TmpKona = 30.0;
        private const int SumML = 340;
        ...
        public Dictionary<string, string> CalculateWordParameters(APIRequest req)
        {
            ...
            kv["SumE1"] = Fmt3(Math.Round(sumE1, 3, MidpointRounding.ToZero)).Replace(".", ",");
            kv["SumE2"] = Fmt2(Math.Round(sumE2, 2, MidpointRounding.ToZero)).Replace(".", ",");
            

            double tmpKonUtr = ((TmpL + Amatt) / TmpKona) + Tmpd;
            double tmpKonstOS = Math.Round(Math.Sin(10.0 * Math.PI / 180.0) * 2.0, 4);
            double tmpKordaInv = Math.Round((Tmpd1 / 2.0) * tmpKonstOS, 1);
        ...
        {
            return Math.Round(
                ((Tmpd - Tmpd1) / 2.0) + ((Amatt + offset) / (2.0 * TmpKona)),
                3);
        }
