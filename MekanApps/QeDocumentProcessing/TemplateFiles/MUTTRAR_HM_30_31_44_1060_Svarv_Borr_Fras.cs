...
{
    public class MUTTRAR_HM_30_31_44_1060_Svarv_Borr_Fras : ITemplateCalculations
    {
        ...
        public Dictionary<string, string> CalculateWordParameters(APIRequest req)
        {
            ...
            double typNum;
            double tmpd4;
            ParseProductDesignation(tmpFormat, out serieInt, out typNum, out tmpd4);
            ...
            kv["Sumd4"] = "(d4) " + Fmt(tmpd4a).ToString(CommonFunctions.Culture).Replace(".", ",");
            kv["Sumd4Tol"] = "± " + Fmt3(GenTol(tmpd4a));

            double tmpdm = DM(tmpd4, stmm);
            kv["Sumdm"] = "(dm) " + Fmt(tmpdm).ToString(CommonFunctions.Culture).Replace(".", ",");
            ...
            double tmpd3 = serieInt == 30 ? D3_30(tmpd4) : D3_31(tmpd4);
            ...
            double tmpd5 = serieInt == 30 ? D5_30(tmpd4, tmpd3) : D5_31(tmpd4, tmpd3);
            ...
            string sumGGTolk = showG3Ganga ? " kontrolleras med tolk  M" + Fmt(tmpG3) + " min/max" : "";
            kv["SumGV"] = "45º";
            ...
            kv["SumN1_4"] = IsInGroup(maskinVal, MachinesAll) ? "Gänga Lyftögla" : "";
            kv["SumB1_4"] = IsInGroup(maskinVal, MachinesAll) ? "G3" : "";
            SumFrequencies(kv, maskinVal);
            SumMeasuringDevices(kv, maskinVal, kv["SumGuTolk"], kv["SumGGTolk"]);
            SumAF(kv, maskinVal);

            kv["SumTextS1"] = EqualsI(maskinVal, "EMAG") ? "Grader, frifläckar, slagmärken, repor, valkar & andra defekter samt ojämnheter kontrolleras med ögonmått, för övriga mått används skjutmått. Funktionskontroll skall göras med hylsa om det finns en sådan" : "Grader, frifläckar, slagmärken, repor, valkar & andra defekter samt ojämnheter kontrolleras med ögonmått, för övriga mått används skjutmått.";
            kv["SumTextS2"] = "Övriga mått kontrolleras vid inställning," + (EqualsI(maskinVal, "EMAG") ? "   Alla muttrar skall vara inoljade på alla ytor" : "");

            string sumRit = serieInt == 30 ? "223015" : "223016";
            ...
            kv["VaLV2X"] = !tmpV21 && !tmpV29 ? "" : "Den valda mallen är endast för standard muttrar använd SPECIAL mallen för HM muttrar";

            return kv;
        }

        private static void ParseProductDesignation(string value, out int series, out double type, out double d4)
        {
            series = 0;
            type = 0;
            d4 = 0;
            ...
            string normalized = Regex.Replace(
                (value ?? string.Empty).Trim().ToUpperInvariant().Replace(',', '.'),
                @"\s+",
                " ");

            Match separated = Regex.Match(
                normalized,
                @"\bHM[\s-]*(?<series>30|31)[\s/.-]+(?<type>\d+(?:\.\d+)?)(?=\s|-|$)");

            if (separated.Success)
            {
                series = TryParseInt(separated.Groups["series"].Value);
                type = TryParseDouble(separated.Groups["type"].Value);
                d4 = type;
                return;
            }
            ...
            Match compact = Regex.Match(
                normalized,
                @"\bHM[\s-]*(?<series>30|31)(?<type>\d{2,3})(?=\s|-|$)");

            if (!compact.Success) return;

            series = TryParseInt(compact.Groups["series"].Value);
            type = TryParseDouble(compact.Groups["type"].Value);
            d4 = type / 2.0 * 10.0;
        }
        ...
        {
            bool emag = EqualsI(maskinVal, "EMAG");
            bool other = EqualsI(maskinVal, "LB45") || EqualsI(maskinVal, "MaxMuller/K&T") || EqualsI(maskinVal, "VTR-160") || EqualsI(maskinVal, "MacTurn 550");
            kv["SumF1_1"] = other ? "1/1" : emag ? "1/10" : "";
            kv["SumF1_2"] = other ? "1/2" : emag ? "1/tim" : "";
            kv["SumF1_3"] = other ? "1/5" : emag ? "1/tim" : "";
            kv["SumF1_4"] = other ? "1/5" : emag ? "1/tim" : "";
            kv["SumF1_5"] = other ? "1/5" : emag ? "1/tim" : "";
            kv["SumF1_6"] = other ? "1/5" : emag ? "1/tim" : "";
            kv["SumF1_7"] = other ? "1/3" : emag ? "1/tim" : "";
            kv["SumF1_8"] = other ? "1/5" : emag ? "inst." : "";
            kv["SumF2_1"] = other ? "1/5" : emag ? "inst." : "";
            kv["SumF2_2"] = other ? "1/5" : emag ? "inst." : "";
            kv["SumF2_3"] = other ? "1/5" : emag ? "1/tim" : "";
            kv["SumF2_4"] = other ? "1/5" : "";
            kv["SumF2_5"] = other || emag ? "inst." : "";
        }

        private static void SumMeasuringDevices(Dictionary<string, string> kv, string maskinVal, string guTolk, string ggTolk)
        {
            bool m = IsInGroup(maskinVal, MachinesAll) || EqualsI(maskinVal, "EMAG");
            bool emag = EqualsI(maskinVal, "EMAG");
            kv["SumD1_1"] = emag ? "UD-apparat" : m ? "Multimar" : "";
            kv["SumD1_2"] = emag ? "Unimeter" : m ? "Mikrometer" : "";
        ...
        private static void SumAF(Dictionary<string, string> kv, string maskinVal)
        {
            ...
            kv["SumAF1_4"] = "";
            kv["SumAF1_5"] = "";
            kv["SumAF1_6"] = isEmag ? "" : (m ? "Bearbetning i OP1 + 3mm" : "");
            kv["SumAF1_7"] = m ? "Övriga Ra värden 6,3" : "";
            kv["SumAF1_8"] = m ? "Vid misstänkt formfel lämnas till mätrum" : "";
            kv["SumAF2_1"] = "";
            kv["SumAF2_2"] = "";
            kv["SumAF2_3"] = isEmag ? "100% kontroll med belyst borrhål" : "";
            kv["SumAF2_4"] = "";
        ...
        private static double D5_30(double d4, double d3)
        {
            if (d4 < 221) return d3 - 18;
            if (d4 < 281) return d3 - 20;
            if (d4 < 341) return d3 - 24;
            if (d4 < 361) return d3 - 26;
            if (d4 < 421) return d3 - 28;
            if (d4 < 501) return d3 - 30;
            if (d4 < 671) return d3 - 40;
            if (d4 < 801) return d3 - 50;
            return d3 - 55;
        }
        private static double D5_31(double d4, double d3)
        {
            if (d4 < 281) return d3 - 30;
            if (d4 < 361) return d3 - 40;
            if (d4 < 381) return d3 - 50;
            if (d4 < 401) return d3 - 60;
            if (d4 < 441) return d3 - 50;
            if (d4 < 461) return d3 - 40;
            if (d4 < 481) return d3 - 60;
            if (d4 < 501) return d3 - 50;
            if (d4 < 601) return d3 - 60;
            if (d4 < 631) return d3 - 70;
            if (d4 < 801) return d3 - 75;
            if (d4 < 851) return d3 - 85;
            if (d4 < 951) return d3 - 90;
            if (d4 < 1001) return d3 - 100;
            return d3 - 90;
        }
        ...
        private static double T7_30(double typ) { if (typ < 84) return 0.84; if (typ < 750) return 1.00; return 1.20; }
        private static double T7_31(double typ) { if (typ < 76) return 0.84; if (typ < 710) return 1.00; return 1.20; }
        ...
        private static double L3Val(double g3)
        {
            if (g3 <= 0) return 0;
            if (Math.Abs(g3 - 10) < 0.001) return 17;
            if (Math.Abs(g3 - 12) < 0.001) return 21;
            if (Math.Abs(g3 - 16) < 0.001) return 27;
            if (Math.Abs(g3 - 20) < 0.001) return 30;
            return 36;
        }
        ...
        private static string FmtComma(double v) => v.ToString("0.###", CultureInfo.InvariantCulture).Replace(".", ",");
