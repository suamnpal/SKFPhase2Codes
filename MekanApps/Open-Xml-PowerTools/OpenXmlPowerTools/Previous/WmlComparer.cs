...
@"<w:p xmlns:w='http://schemas.openxmlformats.org/wordprocessingml/2006/main'>
  ...
  <w:r>
    ...
    WmlComparerSettings settings)
        {
            // ✅ Use OpenXmlPart directly (modern SDK)
            OpenXmlPart partInDeletedDocument = wDocDelta.MainDocumentPart;
            OpenXmlPart partInNewDocument = consolidatedWDoc.MainDocumentPart;

            // ✅ Updated method (must use OpenXmlPart signature)
            ...
            // ✅ Cloning for hashing (unchanged)
            var clonedForHashing = (XElement)CloneBlockLevelContentForHashing(
            ...
            // ✅ Generate hash
            var shaString = clonedForHashing
                            ...
                            .Select(gc =>
                            {
                                ...
                                if (del)
                                {
                                    return (object)gc.Select(gcc =>
                                    {
                                        ...
                                        {
                                            // ✅ Use OpenXmlPart directly
                                            OpenXmlPart sourcePart = openXmlPartOfDeletedContent;
                                            OpenXmlPart targetPart = openXmlPartInNewDocument;
                                ...
                                else if (ins)
                                {
                                    return gc.Select(gcc =>
                                    {
                                        ...
                                        var sourcePart = gc.First().Part;
                                        var targetPart = part;

                                        return gc.Select(gce =>
                                        {
                                            // ✅ Use modern method
                                            return MoveRelatedPartsToDestination(sourcePart, targetPart, newDrawing);
                                        });
                                    });
                                }
                                else
    ...
    XElement contentElement)
        {
            var elementsToUpdate = contentElement
                ...
                .Where(d => d.Attributes()
                    .Any(a => ComparisonUnitWord.s_RelationshipAttributeNames.Contains(a.Name)))
                .ToList();
            ...
            foreach (var element in elementsToUpdate)
            {
                ...
                {
                    string rId = (string)att;

                    // ✅ Get related part safely
                    OpenXmlPart relatedPart;
                    try
                    {
                        relatedPart = sourcePart.GetPartById(rId);
                    }
                    catch
                    {
                    ...
                    // ✅ Add part to target (handles copy + relationship automatically)
                    OpenXmlPart newPart = targetPart.AddPart(relatedPart);

                    // ✅ Get new relationship Id
                    string newRid = targetPart.GetIdOfPart(newPart);

                    // ✅ Update attribute
                    att.Value = newRid;

                    // ✅ Recursive fix for XML parts
                    if (!string.IsNullOrEmpty(newPart.ContentType) &&
                        newPart.ContentType.EndsWith("xml", StringComparison.OrdinalIgnoreCase))
                    {
