                            ...
                            .Select(gc =>
                            {
                                ...
                                if (del)
                                {
                                    return (object) gc.Select(gcc =>
                                    {
                                        ...
                                        {
                                            var deletedPair = openXmlPartOfDeletedContent
                                                .Parts
                                                .FirstOrDefault(p => p.OpenXmlPart.Uri == part.Uri);

                                            var newPair = openXmlPartInNewDocument
                                                .Parts
                                                .FirstOrDefault(p => p.OpenXmlPart.Uri == part.Uri);

                                            if (deletedPair.OpenXmlPart == null || newPair.OpenXmlPart == null)
                                                return null;

                                            return MoveRelatedPartsToDestination(
                                                deletedPair.OpenXmlPart,
                                                newPair.OpenXmlPart,
                                                newDrawing);
                                        });
                                    });
                                ...
                                if (ins)
                                {
                                    return gc.Select(gcc =>
                                    {
                                        ...
                                        return gc.Select(gce =>
                                        {
                                            var deletedPair = openXmlPartOfInsertedContent
                                                .Parts
                                                .FirstOrDefault(p => p.OpenXmlPart.Uri == part.Uri);
