...
namespace OpenXmlPowerTools
{
    ...
    public static class FlatOpc
    {
        ...
        private static XElement GetContentsAsXml(OpenXmlPart part)
        {
                ...
                part.ContentType.EndsWith("xml", StringComparison.OrdinalIgnoreCase))
            {
                ...
                using (XmlReader xr = XmlReader.Create(streamReader))
                {
                    return new XElement(pkg + "part",
                        new XAttribute(pkg + "name", part.Uri.ToString()),
            ...
            else
            {
                ...
                using (BinaryReader binaryReader = new BinaryReader(str))
                {
                    ...
                    // Base64 with 76-char line chunks
                    string base64String = Convert.ToBase64String(byteArray)
                        .Select((c, i) => new FlatOpcTupple
                        {
                            FoCharacter = c,
                            FoChunk = i / 76
                        })
                        .GroupBy(c => c.FoChunk)
                        .Select(g => new string(g.Select(x => x.FoCharacter).ToArray()))
        ...
        public static XmlDocument OpcToXmlDocument(string fileName)
        {
            ...
            {
                OpenXmlPackage docPkg;

                // ✅ Detect document type
                try
                {
                    docPkg = WordprocessingDocument.Open(stream, false);
                }
                catch
                {
                    stream.Position = 0;
                    try
                    {
                        docPkg = SpreadsheetDocument.Open(stream, false);
                    }
                    catch
                    {
                        stream.Position = 0;
                        try
                        {
                            docPkg = PresentationDocument.Open(stream, false);
                        }
                        catch
                        {
                ...
                using (docPkg)
                {
                    ...
                    XDocument doc = new XDocument(
                        ...
                        new XElement(pkg + "package",
                            ...
                            // ✅ Works because docPkg is OpenXmlPackage
                            docPkg.Parts.Select(p => GetContentsAsXml(p.OpenXmlPart))
                        )
                    );
        ...
        {
            using (var stream = File.OpenRead(fileName))
            using (var docPkg = OpenDocument(stream))
            {
                XNamespace pkg = "http://schemas.microsoft.com/office/2006/xmlPackage";
        ...
        private static OpenXmlPackage OpenDocument(Stream stream)
        {
            try
            {
            ...
            using (var zip = new ZipArchive(File.Create(outputPath), ZipArchiveMode.Create))
            {
                ...
                foreach (var xmlPart in doc.Root.Elements())
                {
                    ...
                    if (contentType == "application/vnd.openxmlformats-package.relationships+xml")
                        continue; // relationships handled later

                    string entryName = name.TrimStart('/'); // Zip uses relative paths
                    ...
                    using (var stream = entry.Open())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                string base64 = (string)xmlPart.Element(pkg + "binaryData");
                ...
                // ✅ Second pass: create relationships (.rels files)
                foreach (var xmlPart in doc.Root.Elements())
                {
                    string contentType = (string)xmlPart.Attribute(pkg + "contentType");
                    string name = (string)xmlPart.Attribute(pkg + "name");
                    ...
                    string entryName = name.TrimStart('/');
                    ...
                    using (var writer = XmlWriter.Create(stream))
                    {
                        var rels = new XElement(rel + "Relationships",
                            xmlPart
                                .Descendants(rel + "Relationship")
                                .Select(r => new XElement(rel + "Relationship",
                                    new XAttribute("Id", (string)r.Attribute("Id")),
                                    new XAttribute("Type", (string)r.Attribute("Type")),
                                    new XAttribute("Target", (string)r.Attribute("Target")),
                                    r.Attribute("TargetMode") != null
                                        ? new XAttribute("TargetMode", (string)r.Attribute("TargetMode"))
                                        : null
                                ))
                        );
...
listSeparator
        ...
        public class ImageData
        {
            ...
            internal List<ContentPartRelTypeIdTuple> ContentPartRelTypeIdList { get; }
                    = new List<ContentPartRelTypeIdTuple>();
            ...
            public bool Compare(ImageData arg)
            {
                ...
                if (Image.Length != arg.Image.Length)
                    return false;

                // ✅ Efficient byte comparison
                return Image.SequenceEqual(arg.Image);
            }
        }
        ...
        internal class MediaData
        {
            ...
            public bool Compare(MediaData arg)
            {
                ...
                if (Media.Length != arg.Media.Length)
                    return false;

                return Media.SequenceEqual(arg.Media);
            }
        }
