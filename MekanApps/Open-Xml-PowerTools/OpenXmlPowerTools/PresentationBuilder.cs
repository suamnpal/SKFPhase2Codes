...
namespace OpenXmlPowerTools
{
    ...
    public static class PresentationBuilder
    {
        ...
        private static XElement CreatedEmbeddedFontPart(
     ...
     XName fontXName)
        {
            // ✅ Safe rel lookup
            var relAttr = font.Element(fontXName)?.Attribute(R.id);
            if (relAttr == null)
                return null;

            string oldRelId = relAttr.Value;

            // ✅ Get source font part
            FontPart oldFontPart = (FontPart)sourceDocument
                .PresentationPart
                .GetPartById(oldRelId);

            // ✅ Create new font part using content type (modern approach)
            FontPart newFontPart = newDocument
                .PresentationPart
                .AddFontPart(oldFontPart.ContentType);
            ...
            // ✅ Get relationship ID assigned by SDK
            string newId = newDocument.PresentationPart.GetIdOfPart(newFontPart);

            // ✅ Return updated XML
            return new XElement(fontXName,
        ...
        private static void CopyNotesMaster(
     PresentationDocument sourceDocument,
     PresentationDocument newDocument,
     List<ImageData> images,
     List<MediaData> mediaList)
        {
            var sourcePresentationPart = sourceDocument.PresentationPart;
            var targetPresentationPart = newDocument.PresentationPart;

            XDocument newPresentation = targetPresentationPart.GetXDocument();
            XDocument oldPresentationDoc = sourcePresentationPart.GetXDocument();

            // ✅ Copy notesSz safely
            XElement oldNotesSz = oldPresentationDoc.Root.Element(P.notesSz);
            XElement newNotesSz = newPresentation.Root.Element(P.notesSz);

            if (oldNotesSz != null)
            {
                if (newNotesSz != null)
                    newNotesSz.ReplaceWith(new XElement(oldNotesSz));
                else
                    newPresentation.Root.Add(new XElement(oldNotesSz));
            }

            // ✅ Copy Notes Master
            if (sourcePresentationPart.NotesMasterPart != null)
            {
                NotesMasterPart oldMaster = sourcePresentationPart.NotesMasterPart;

                // ✅ Use AddPart instead of AddNewPart (better for copying relationships)
                NotesMasterPart newMaster = targetPresentationPart.AddPart(oldMaster);

                // ✅ Copy Theme safely
                if (oldMaster.ThemePart != null)
                {
                    ThemePart oldTheme = oldMaster.ThemePart;
                    ThemePart newTheme = newMaster.AddPart(oldTheme);

                    // ✅ Optional: update content if needed
                    newTheme.PutXDocument(oldTheme.GetXDocument());

                    CopyRelatedPartsForContentParts(
                        newDocument,
                        oldTheme,
                        newTheme,
                        new[] { newTheme.GetXDocument().Root },
                        images,
                        mediaList);
                }

                // ✅ Copy Notes Master XML
                newMaster.PutXDocument(oldMaster.GetXDocument());

                // ✅ Copy relationships using OpenXmlPart logic (must already be modernized)
                AddRelationships(oldMaster, newMaster, new[] { newMaster.GetXDocument().Root });
                ...
                CopyRelatedPartsForContentParts(
                    ...
                    oldMaster,
                    newMaster,
                    new[] { newMaster.GetXDocument().Root },
                    images,
                    mediaList);

                // ✅ Ensure notesMasterIdLst exists
                XElement notesMasterIdList = newPresentation.Root.Element(P.notesMasterIdLst);
                if (notesMasterIdList == null)
                {
                    notesMasterIdList = new XElement(P.notesMasterIdLst);
                    newPresentation.Root.Add(notesMasterIdList);
                }

                // ✅ Add reference
                notesMasterIdList.Add(
                    new XElement(P.notesMasterId,
                        new XAttribute(R.id, targetPresentationPart.GetIdOfPart(newMaster))
                    )
                );
            }
        }
            ...
            foreach (XElement dataReference in newChart.GetXDocument().Descendants(C.externalData))
            {
                ...
                if (oldPartIdPair != null)
                {
                    ...
                    if (oldEmbeddedObjectPart != null)
                    {
                        ...
                        using (Stream newObject = newPart.GetStream(FileMode.Create, FileAccess.Write))
                        {
                            oldObject.CopyTo(newObject);   // ✅ simpler + safer
                        }

                        string rId = newChart.GetIdOfPart(newPart);
                        dataReference.Attribute(R.id).Value = rId;

                        // ✅ FIX: adjust relationship type using OpenXmlPart APIs only

                        var rel = newChart.HyperlinkRelationships
                            .FirstOrDefault(r => r.Id == rId);

                        if (rel == null)
                        {
                            // remove wrong relationship
                            newChart.DeletePart(newPart);

                            // recreate properly as external-style relationship with correct type
                            var fixedRel = newChart.AddExternalRelationship(
                                "http://schemas.openxmlformats.org/officeDocument/2006/relationships/oleObject",
                                new Uri("embeddedObject", UriKind.Relative)
                            );

                            dataReference.Attribute(R.id).Value = fixedRel.Id;
                        }
            ...
            List<ImageData> images)
        {
            ...
            {
                ExternalRelationship er = oldContentPart.ExternalRelationships
     .FirstOrDefault(r => r.Id == relId);

                if (er != null)
                {
                    ExternalRelationship newEr = newContentPart.AddExternalRelationship(
                        er.RelationshipType,
                        er.Uri);
                ...
                {
                    // ✅ Safe fallback: remove invalid reference
                    imageReference.Attribute(R.id)?.Remove();
                }
            }
        }
        ...
        private static void CopyExtendedPart(OpenXmlPart oldContentPart, OpenXmlPart newContentPart, XElement extendedReference, XName attributeName)
        {
            ...
            catch (ArgumentOutOfRangeException)
            {
                ...
                {
                    // ✅ Safe fallback: remove invalid reference instead of corrupting package
                    extendedReference.Attribute(R.id)?.Remove();
                }
            }
        }
