...
namespace OpenXmlPowerTools
{
    ...
    public static class DocumentBuilder
    {
        ...
        public static IEnumerable<WmlDocument> SplitOnSections(WmlDocument doc)
        {
                    ...
                    .Rollup(new Atbid
                    {
                        ...
                        (b, p) =>
                        {
                            ...
                            if (elementBefore != null &&
                                elementBefore.Descendants(W.sectPr).Any())
                            {
                                return new Atbid
                                {
            ...
            // ✅ Important: build documents outside the stream scope
  ...
  <w:docParts>
    <w:docPart>
      ...
      <w:docPartBody>
        ...
        static ImagePart AddImagePartToContainer(OpenXmlPart container, string contentType)
        {
            if (container is MainDocumentPart m)
                return m.AddImagePart(contentType);

            if (container is HeaderPart h)
                return h.AddImagePart(contentType);

            if (container is FooterPart f)
                return f.AddImagePart(contentType);

            if (container is EndnotesPart e)
                return e.AddImagePart(contentType);

            if (container is FootnotesPart fn)
                return fn.AddImagePart(contentType);

            if (container is ThemePart t)
                return t.AddImagePart(contentType);

            if (container is WordprocessingCommentsPart c)
                return c.AddImagePart(contentType);

            if (container is DocumentSettingsPart ds)
                return ds.AddImagePart(contentType);

            if (container is ChartPart cp)
                return cp.AddImagePart(contentType);

            if (container is NumberingDefinitionsPart nd)
                return nd.AddImagePart(contentType);

            if (container is DiagramDataPart dd)
                return dd.AddImagePart(contentType);

            if (container is ChartDrawingPart cd)
                return cd.AddImagePart(contentType);

            throw new NotSupportedException(
                "Unsupported container type: " + container.GetType().Name);
        }

        static void CopyRelatedImage(
     ...
     List<ImageData> images)
        {
            ...
            // ✅ Skip if already exists in new part
            if (newContentPart.Parts.Any(p => p.RelationshipId == relId))
                return;

            if (newContentPart.ExternalRelationships.Any(er => er.Id == relId))
                return;

            // ✅ Try internal relationship
            var relPair = oldContentPart.Parts
                .FirstOrDefault(p => p.RelationshipId == relId);

            if (relPair != null)
            {
                if (!(relPair.OpenXmlPart is ImagePart oldImagePart))
                    throw new DocumentBuilderException(
                        "Invalid document - target part is not ImagePart");

                // ✅ Deduplication
                ImageData imageData = ManageImageCopy(oldImagePart, newContentPart, images);

                if (imageData.ImagePart == null)
                {
                    // ✅ Create correct image part dynamically
                    ImagePart newImagePart = AddImagePartToContainer(newContentPart, oldImagePart.ContentType);

                    imageData.ImagePart = newImagePart;

                    // ✅ Assign relationship id
                    string newRid = newContentPart.GetIdOfPart(newImagePart);

                    imageData.AddContentPartRelTypeResourceIdTupple(
                        newContentPart,
                        newImagePart.RelationshipType,
                        newRid);

                    imageReference.Attribute(attributeName).Value = newRid;

                    // ✅ Copy binary content
                    imageData.WriteImage(newImagePart);
                }
                else
                {
                    // ✅ Reuse already copied image
                    var existing = imageData.ContentPartRelTypeIdList
                        .FirstOrDefault(x => x.ContentPart == newContentPart);

                    if (existing != null)
                    {
                        imageReference.Attribute(attributeName).Value = existing.RelationshipId;
                        return;
                    }

                    // ✅ Create new relationship to existing part
                    string newId = "rId" + Guid.NewGuid().ToString("N");

                    newContentPart.CreateRelationshipToPart(imageData.ImagePart, newId);
            ...
            else
            {
                ...
                var extRel = oldContentPart.ExternalRelationships
                    .FirstOrDefault(er => er.Id == relId);

                if (extRel != null)
                {
                    var newExt = newContentPart.AddExternalRelationship(
                        extRel.RelationshipType,
                        extRel.Uri);

                    imageReference.Attribute(attributeName).Value = newExt.Id;
                    return;
                }

                throw new DocumentBuilderInternalException(
                    "Source is unsupported document - contains reference to NULL image");
            }
        }
