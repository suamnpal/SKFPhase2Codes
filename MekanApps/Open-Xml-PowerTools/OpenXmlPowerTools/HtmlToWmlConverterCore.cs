     ...
     WordprocessingDocument wDoc)
        {
            ...
            // ✅ Let SDK generate relationship ID automatically
            ImagePart newPart = mdp.AddImagePart(ImagePartType.Png);

            using (Stream s = newPart.GetStream(FileMode.Create, FileAccess.Write))
            {
                s.Write(ba, 0, ba.Length);   // ✅ fixed length usage
            }

            string rId = mdp.GetIdOfPart(newPart);   // ✅ correct way
            ...
            string pictureDescription = "Picture " + pictureId;
