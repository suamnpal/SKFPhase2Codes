...
namespace OpenXmlPowerTools
{
    ...
    public static class WmlToHtmlConverter
    {
        ...
        private static string NormalizeCss(string key, string value)
        {
            ...
            {
                var num = value.Replace("pt", "").Trim();
                if (decimal.TryParse(num, out var pt))
                {
                    var px = pt * 1.333m;
                    value = $"{px:0}px";
                }
            ...
            if (key == "display" && value.Contains("inline-block"))
                return "";

            if (key == "margin-bottom" && value == ".001pt")
                return "";

            return $"{key}:{value};";
        }

        private static void ReifyStylesAndClasses(WmlToHtmlConverterSettings settings, XElement xhtml)
        {
            foreach (var d in xhtml.DescendantsAndSelf())
            {
                ...
                var styleValue = string.Join("",
                    style
                    .Where(p => p.Key != "PtStyleName")
                    .OrderBy(p => p.Key)
                    .Select(e => NormalizeCss(e.Key, e.Value)));
                ...
                if (!string.IsNullOrEmpty(styleValue))
                {
                    if (d.Attribute("style") != null)
                        ...
                        d.SetAttributeValue("style", styleValue);
                }
            }
        }
            ...
            decimal currentMarginLeft)
        {
            ...
            {
                return ProcessImage(wordDoc, element, settings);
            }

            if (element.Name.LocalName == "AlternateContent" ||  element.Name.LocalName == "Choice" || element.Name.LocalName == "Fallback")
            {
        ...
        private static void ApplyTableProperties(XElement tbl, XElement table)
        {
            var tblPr = tbl.Element(W.tblPr);
            if (tblPr == null)
                return;

            // ✅ Ensure base table style
            var baseStyle = "border-collapse:collapse;width:100%;table-layout:fixed;";
            var existingStyle = table.Attribute("style")?.Value ?? "";

            if (!existingStyle.Contains("border-collapse"))
                existingStyle = baseStyle + existingStyle;

            table.SetAttributeValue("style", existingStyle);

            // ✅ RTL support
            if (tblPr.Element(W.bidiVisual) != null)
            {
                //table.SetAttributeValue("dir", "rtl");
                //table.SetAttributeValue("style",
                //    table.Attribute("style").Value + "direction:rtl;");
            }
            ...
            if (tblBorders == null)
            {
                // ✅ fallback border (important for Power Apps)
                table.SetAttributeValue("style",
                    table.Attribute("style").Value + "border:1px solid #000;");
                return;
            }

            var style = new StringBuilder();

            AppendBorder(tblBorders.Element(W.top), "top", style);
            AppendBorder(tblBorders.Element(W.right), "right", style);
            AppendBorder(tblBorders.Element(W.bottom), "bottom", style);
            AppendBorder(tblBorders.Element(W.left), "left", style);

            if (style.Length > 0)
            {
                var current = table.Attribute("style")?.Value ?? "";
                var separator = current.EndsWith(";") ? "" : ";";

                table.SetAttributeValue("style",
                    current + separator + style.ToString());
            }
        }

        private static string BuildCellStyle(XElement tc)
        {
            var style = new StringBuilder();
            ...
            // Base style
            style.Append("padding:5px;");

            if (tcPr == null) return style.ToString();

            // ✅ Borders
            var tcBorders = tcPr.Element(W.tcBorders);
            AppendBorder(tcBorders?.Element(W.top), "top", style);
            AppendBorder(tcBorders?.Element(W.right), "right", style);
            AppendBorder(tcBorders?.Element(W.bottom), "bottom", style);
            AppendBorder(tcBorders?.Element(W.left), "left", style);
            ...
            // ✅ Vertical alignment
            var vAlign = tcPr.Element(W.vAlign);
            if (vAlign != null)
            {
                var val = (string)vAlign.Attribute(W.val);
                if (val == "center") style.Append("vertical-align:middle;");
                else if (val == "bottom") style.Append("vertical-align:bottom;");
            ...
            if (tcW != null)
            {
                var type = (string)tcW.Attribute(W.type);
                var w = (int?)tcW.Attribute(W._w);

                if (w != null)
                {
                    if (type == "dxa")
                        style.Append($"width:{w / 20}px;");
                    else if (type == "pct")
                        style.Append($"width:{w / 50}%;");
                }
            }
        ...
        private static void AppendBorder(XElement border, string side, StringBuilder style)
        {
            if (border == null)
            {
                style.Append($"border-{side}:none;");
                return;
            }

            var val = (string)border.Attribute(W.val);
            var sz = (int?)border.Attribute(W.sz);
            var color = (string)border.Attribute(W.color);
            ...
            if (val == "dashed") cssStyle = "dashed";
            if (val == "dotted") cssStyle = "dotted";
            if (val == "double") cssStyle = "double";

            string cssColor = (color == null || color == "auto")
                ? "black"
                : $"#{color}";

            decimal width = Math.Max(1, (sz ?? 8) / 8m);

            style.Append($"border-{side}:{cssStyle} {cssColor} {width}px;");
        }

        private static int ApplyColSpan(XElement tc, XElement td)
        {
            var span = tc.Element(W.tcPr)?
                .Element(W.gridSpan)?
                .Attribute(W.val);

            int colspan = span != null ? int.Parse(span.Value) : 1;

            if (colspan > 1)
                td.Add(new XAttribute("colspan", colspan));

            return colspan;
        }
        private static bool ApplyRowSpan(XElement tbl, XElement tr, XElement tc, XElement td,  Dictionary<int, int> map,
    int colIndex)
        {
            var vMerge = tc.Element(W.tcPr)?.Element(W.vMerge);
            ...
            if (val == "restart")
            {
                ...
                foreach (var nextRow in tr.ElementsAfterSelf(W.tr))
                {
                    var nextCell = nextRow.Elements(W.tc).ElementAtOrDefault(colIndex);
                    if (nextCell?.Element(W.tcPr)?.Element(W.vMerge) == null)
                        break;

                    span++;
                }

                td.Add(new XAttribute("rowspan", span));
                map[colIndex] = span - 1;
            }
            else
            {
        ...
        private static int SkipRowSpans(Dictionary<int, int> map, int index)
        {
            while (map.ContainsKey(index) && map[index] > 0)
            {
                map[index]--;
                index++;
            }
        ...
        private static XElement ProcessTable(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings settings, XElement tbl,
    decimal currentMarginLeft)
        {
            var table = new XElement(Xhtml.table, new XAttribute("style", "border-collapse:collapse;width:100%;table-layout:fixed;"));

            ApplyTableProperties(tbl, table);

            var rowspanMap = new Dictionary<int, int>();

            foreach (var tr in tbl.Elements(W.tr))
            {
                var trElem = new XElement(Xhtml.tr);
                int colIndex = 0;

                foreach (var tc in tr.Elements(W.tc))
                {
                    colIndex = SkipRowSpans(rowspanMap, colIndex);

                    var td = new XElement(Xhtml.td);

                    int colspan = ApplyColSpan(tc, td);
                    bool skip = ApplyRowSpan(tbl, tr, tc, td, rowspanMap, colIndex);
                    ...
                    td.SetAttributeValue("style", BuildCellStyle(tc));

                    AddCellContent(wordDoc, settings, tc, td, currentMarginLeft);
            ...
            return WrapTable(tbl, table);
        }

        private static void AddCellContent(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings settings, XElement tc, XElement td,
    decimal currentMarginLeft)
        {
            foreach (var child in tc.Elements())
            {
                if (child.Name == W.tbl)
                {
                    td.Add(ProcessTable(wordDoc, settings, child, currentMarginLeft));
                }
                else
                {
                    td.Add(ConvertToHtmlTransform(wordDoc, settings, child, false, currentMarginLeft));
                }
            }
        }

        private static XElement WrapTable(XElement tbl, XElement table)
        {
            var jc = (string)tbl.Element(W.tblPr)?
                              .Element(W.jc)?
                              .Attribute(W.val) ?? "left";

            return new XElement(Xhtml.div,
                new XAttribute("style", $"text-align:{jc};"),
                table);
        }
        ...
        private static void FixTopBorder(XElement[][] ta, XElement thisCell, int x, int y)
        {
            ...
            {
                var rowAbove = ta[y - 1];
                if (x < rowAbove.Length)
                {
                    FixBorder(thisCell, rowAbove[x], W.top, W.bottom);
                }
            }
        }
        ...
        private static void FixLeftBorder(XElement[][] ta, XElement thisCell, int x, int y)
        {
            if (x > 0)
            {
                FixBorder(thisCell, ta[y][x - 1], W.left, W.right);
            }
        }
        private static void FixBottomBorder(XElement[][] ta, XElement thisCell, int x, int y)
        {
            ...
            {
                var rowBelow = ta[y + 1];
                if (x < rowBelow.Length)
                {
                    FixBorder(thisCell, rowBelow[x], W.bottom, W.top);
                }
            }
        }
        ...
        private static void FixRightBorder(XElement[][] ta, XElement thisCell, int x, int y)
        {
            if (x < ta[y].Length - 1)
            {
                FixBorder(thisCell, ta[y][x + 1], W.right, W.left);
            }
        }

        private static void FixBorder(XElement thisCell, XElement neighborCell, XName thisSide, XName neighborSide)
        {
            if (thisSide == null || neighborSide == null)
                return; // prevent crash

            var thisBorders = GetBorders(thisCell);
            var neighborBorders = GetBorders(neighborCell);

            if (thisBorders == null || neighborBorders == null)
                return;

            var thisBorder = thisBorders.Element(thisSide);
            var neighborBorder = neighborBorders.Element(neighborSide);

            if (thisBorder != null && neighborBorder != null)
            {
                ResolveCellBorder(thisBorder, neighborBorder);
            }
        }

        private static XElement GetBorders(XElement cell)
        {
            return cell?.Element(W.tcPr)?.Element(W.tcBorders);
        }
        ...
        private static void ResolveCellBorder(XElement border1, XElement border2)
        {
            ...
            // ✅ CRITICAL FIX
            if (string.IsNullOrEmpty(border1Val) || string.IsNullOrEmpty(border2Val))
                return;

            if (border1Val == "nil" || border2Val == "nil")
                return;

            var border1SzAttr = border1.Attribute(W.sz);
            var border2SzAttr = border2.Attribute(W.sz);

            if (border1SzAttr == null || border2SzAttr == null)
                return;
            ...
            decimal border1Size = (decimal)border1SzAttr;
            decimal border2Size = (decimal)border2SzAttr;

            if (border1Size > border2Size)
            {
                BorderOverride(border1, border2);
                return;
            }
            ...
            var border1Type = border1Val;
            var border2Type = border2Val;

            if (!string.IsNullOrEmpty(border1Type) &&
                !string.IsNullOrEmpty(border2Type) &&
                BorderTypePriority.ContainsKey(border1Type) &&
            ...
            var color1Str = (string)border1.Attribute(W.color) ?? "000000";
            var color2Str = (string)border2.Attribute(W.color) ?? "000000";

            if (color1Str == "auto") color1Str = "000000";
            if (color2Str == "auto") color2Str = "000000";
        ...
        public static XElement ProcessImage(WordprocessingDocument wordDoc,
            XElement element, WmlToHtmlConverterSettings settings)
        {
            ...
            {
                return ProcessDrawing(wordDoc, element, settings);
            }
            if (element.Name == W.pict || element.Name == W._object)
            {
                return ProcessPictureOrObject(wordDoc, element, settings.ImageHandler);
            }
            return null;
        }

        private static XElement ProcessShape(
    WordprocessingDocument wordDoc,
    XElement containerElement,
    WmlToHtmlConverterSettings settings)
        {
            var textBoxContent = containerElement
                .Descendants()
                .FirstOrDefault(x => x.Name.LocalName == "txbxContent");
            ...
            var extent = containerElement
                .Descendants(WP.extent)
                .FirstOrDefault();

            if (extent != null)
            {
                long cx = (long?)extent.Attribute("cx") ?? 0;
                long cy = (long?)extent.Attribute("cy") ?? 0;
            ...
            // Fill Color
            string fillColor = "#EAEAEA";

            var schemeClr = containerElement
                .Descendants(A.schemeClr)
                .FirstOrDefault();

            if (schemeClr != null)
            {
                switch ((string)schemeClr.Attribute("val"))
                {
                    case "accent1": fillColor = "#156082"; break;
                    case "accent2": fillColor = "#E97132"; break;
                    case "accent3": fillColor = "#196B24"; break;
                    case "accent4": fillColor = "#0F9ED5"; break;
                    case "accent5": fillColor = "#A02B93"; break;
                    case "accent6": fillColor = "#4EA72E"; break;
                }
            }
            ...
            string.IsNullOrWhiteSpace(shapeText) ? " " : shapeText);
        }
            ...
            XElement element, WmlToHtmlConverterSettings settings)
        {
            ...
            var graphicData = containerElement.Descendants(A.graphicData).FirstOrDefault();
            if (graphicData != null)
            {
                var isShape = graphicData.Attribute("uri")?.Value == "http://schemas.microsoft.com/office/word/2010/wordprocessingShape";
                if (isShape)
                {
                    return ProcessShape(
                        wordDoc,
            ...
            using (var bitmap = new Bitmap(partStream))
            {
                if (extentCx != null && extentCy != null)
                {
                    ...
                    var imgElement2 = settings.ImageHandler(imageInfo);
                ...
                var imgElement = settings.ImageHandler(imageInfo2);
                if (hyperlinkUri != null)
                {
                    return new XElement(XhtmlNoNamespace.a,
                        new XAttribute(XhtmlNoNamespace.href, hyperlinkUri),
