...
namespace PAHelperFunctionsApp.HelperFunctions
{
    using System;
    using System.Collections.Generic;
    ...
    internal class Code39Barcode
    {
        ...
        public Bitmap Create(int height, string content, int padding = 0, bool includeCode = false, int barWidth = 1)
        {
            ...
            if (includeCode)
            {
                var textLayerSize = graphics.MeasureString(content, new Font("Tahoma", 14));
                var textLayerSizeWidth = (int)Math.Ceiling(textLayerSize.Width);

                textLayerSizeHeight = (int)Math.Ceiling(textLayerSize.Height);

                graphics.DrawString(content, new Font("Tahoma", 14), Brushes.Black,
